...
工地不安全行为检测系统（YOLOv8）：RTSP 相机取流 → 多模型检测 → 规则判定 → 快照/告警入库，
...
# 用测试配置跑（真实相机已置为停用，另有本地视频相机 CAM_TEST_1）
...
| 页面 | 功能 |
|------|------|
| 总览 | 相机在线状态、实时告警流、7 天趋势、磁盘水位 |
| 相机管理 | 增删改/启停/重连，**全部热生效**（不重启主程序） |
...
| 规则配置 | 规则模板化在线编辑（参数/绑定模型/分配相机），存 `config/rules.yaml` |
| 检测测试台 | 上传图片（或取相机帧）用已加载模型试跑，出标注图+JSON |
